...
- **그룹 동질성** (두 가지):
  1. **그룹 간 VM 대수**: ev01/ev02/ev03 그룹의 VM 대수가 서로 같아야 함. **PASS인 VM도 포함해서 조회한 VM 전부**로 셉니다 — PASS인 VM은 교정 대상에서 빠지지만 그룹 구성에는 속하기 때문입니다(예: ev01 PASS 1 + FAIL 1 / ev02 FAIL 2는 2:2 구성이라 통과).
  2. **그룹 내 스펙**: 같은 그룹의 **교정 대상끼리** vCPU/메모리/디스크/CPU Shares/HT가 같아야 함. **코어/소켓과 NUMA는 비교하지 않습니다** — 둘 다 `-fix`가 기대값으로 직접 고치는 항목이라 FAIL인 VM은 값이 다를 수밖에 없고, 비교하면 고치려는 바로 그 불일치 때문에 교정이 막히기 때문입니다. 반대로 메모리/디스크/Shares는 이 도구가 못 고치는(수동조치) 항목이라 다르면 진짜로 다른 스펙이므로 계속 비교합니다.
