...
| `VMsetup/affinity_setting-source/main.go` | affinity 일괄 적용(`-vm_cnt` 1~10, ev마다 파일 필수 — 자동 계산 없음) |
...
| `vm-param-check-usability-improvement/vm-param-check/main.go` | 체크/`-fix` CLI 진입점, 플래그 등록(ev01~ev10), `-specRoot`, `-specExport` |
| `.../vm-param-check/model/group.go` | ev 그룹 판정(`ClassifyGroup`)과 최대 개수(`MaxGroup=10`) — 그룹을 늘리려면 여기부터 |
...
- **"ev 개수를 늘려달라"** → `vm-param-check/model/group.go`(MaxGroup) + 각 도구의 `maxVMCount`/`maxGroup` 상수 + `vm_setup.sh`의 `seq 1 10`
