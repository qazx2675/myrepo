...
**포트그룹만 바꾸기** (IP·LDAP 는 건너뜀, `GOSSH_PW` 불필요):

```bash
export VC_PASSWORD='********'
./change.sh --only E web        # 전처리(vswitch 표준화) → 포트그룹 변경
./change.sh port -u web         # 이미 전처리(work/vswitch_web.std.txt)가 있을 때 포트그룹만
                                # (port 뒤에 -u 없이 이름만 쓰면 인시던트 이름으로 해석됩니다)
./change.sh port <인시던트>      # 저장된 인시던트의 포트그룹 이어서 진행
```

### 2.3.1 진행 화면과 Ctrl+C (IP·LDAP 단계)

IP 변경·LDAP 설정 단계가 **60초 안에 끝나면 화면은 예전과 똑같습니다.** 60초가
넘으면 화면이 진행 목록으로 바뀝니다(vim/less 처럼 별도 화면 — 끝나면 원래
화면으로 돌아오고 그 위에 결과가 이어서 찍힙니다).

```
=== IP 변경 진행 중 === 경과 00:01:23
완료 12/300 (4%)  진행/대기 288  실패 1
↑/↓ 선택  ←/→ 또는 PgUp/PgDn 페이지  Enter 상세  Ctrl+C 3회(5초 안) 종료   [페이지 1/13]

  web0001                  완료(OK)         00:00:12
> web0002                  진행/대기        00:01:23
  web0003                  실패(접속)       00:00:03
```

| 키 | 동작 |
|---|---|
| ↑ / ↓ | 호스트 선택 (페이지 끝에서 다음/이전 페이지로 넘어감) |
| ← / → 또는 PgUp / PgDn | 이전/다음 페이지 (한 페이지 줄 수는 터미널 높이에 맞춤) |
| Enter | 선택한 호스트의 상세(원격 출력, 오류) 보기 — 아무 키나 누르면 목록으로 |
| Ctrl+C | **5초 안에 3번** 눌러야 즉시 종료 (1~2번째는 안내만) |

- 끝난 호스트의 경과 시간은 끝난 시점에서 멈춥니다.
- gossh 는 호스트 한 대가 끝나야 결과를 보내므로 "진행 중"과 "동시접속 제한으로
  대기 중"을 구분할 수 없어 둘을 **진행/대기** 로 표시합니다.
- 1000대 이상이어도 한 페이지만 그리고 바뀐 줄만 덮어써서 가볍습니다.
- 화면은 제어 터미널(`/dev/tty`)에 그리므로 엔진 출력·로그 형식은 바뀌지 않습니다.
  터미널이 없는 실행(cron, nohup)은 화면 없이 예전처럼 동작합니다.

**Ctrl+C 3회 종료** 시 되돌리기는 하지 않습니다. 진행 중이던 gossh 를 끝내고,
끝난 호스트(OK/실패)와 **안 끝난 호스트**(변경이 일부 적용됐을 수 있음 — 직접
확인 필요) 목록을 화면과 로그에 남긴 뒤 `B1` 로 멈춥니다(다음 단계로 넘어가지
않음). 1~2번만 누르고 5초가 지나면 횟수가 초기화되고 작업은 계속됩니다.

**느린 VM(다른 VM 이 CPU 를 과점유, shares 1:10000 등):** 로그인에만 수 시간이
걸려도 살아 있으면(ping 응답) 2차에서 최대 48시간(`timeout_pass2`)까지 기다려
끝까지 변경합니다(3.5절). 그동안 진행 화면으로 상태를 볼 수 있습니다.
...
| `change.sh port -u <계정>` / `change.sh port <인시던트>` | 포트그룹(2차)만 (`-u` 없이 쓴 이름은 인시던트 이름) |
| `change.sh --only E <계정>` | 포트그룹만 — 전처리(vswitch 표준화)부터 새로 |
...
`gossh -t` 는 **로그인(접속+인증)까지** 기다리는 시간입니다(로그인 뒤 명령 실행
에는 제한이 없음). 크게 잡으면 죽은 노드에서도 그만큼 대기하므로:

1. **1차** — `timeout_pass1`(기본 60초)로 전체 실행
2. 실패한 호스트에 **ping 선검사** — 3번 중 한 번이라도 응답하면 살아 있음
   (CPU 를 거의 못 받는 VM 도 놓치지 않도록). 여러 대를 **동시에** 검사합니다.
3. **2차** — `ping` 되는 호스트만 `timeout_pass2`(기본 **172800초 = 48시간**)로 재실행.
   다른 VM 에 CPU 를 과점유당해 로그인에 수 시간이 걸리는 VM 도 끝까지 기다립니다.
   60초가 넘으면 진행 화면(2.3.1)이 뜹니다.
4. `ping` 도 안 되면 즉시 실패 확정 (대기 안 함)

> 2026-09-23 이전에 만든 `integration.conf` 에는 `timeout_pass2 = 600` 이 들어
> 있습니다. 48시간 대기를 쓰려면 172800 으로 바꾸십시오(`update.sh` 가 알려줍니다).

LDAP 은 사이트별로 스크립트가 달라 사이트마다 gossh 를 한 번씩 돌리는데, 이제
**사이트들을 동시에** 돌립니다(예전엔 순서대로라 한 사이트의 느린 VM 이 다른 사이트
전체를 막았음). 결과 출력 순서는 예전과 같습니다.
...
| `ip_ok_<계정>.txt` / `ldap_ok_<계정>.txt` | 단계별 성공. LDAP 이 **이미 원하는 설정이라 바꾼 게 없는(NOCHANGE)** VM 은 성공으로 치지만 이번에 바꾼 게 없으므로 `ldap_ok`(롤백 대상)에는 넣지 않습니다 |
