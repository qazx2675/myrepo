...
```bash
# 서버 배치: 네 폴더를 같은 부모 폴더 아래에 나란히 둔다 (README.md 등 문서는 복사하지 않아도 됨)
...
| 명령 | 동작 |
|---|---|
| `bash setup.sh` | 11개 도구를 모두 빌드하고 성공/실패를 요약 (하나라도 실패하면 종료코드 1) |
| `bash setup.sh vm_create nic_assign` | 지정한 도구만 빌드 |
| `bash setup.sh -l` | 빌드 대상과 실행 파일 경로 목록 출력 |
| `bash setup.sh -c` | 빌드된 실행 파일과 vendor 링크 정리 |
