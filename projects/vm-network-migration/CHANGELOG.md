...
## 2026-09-15 (추가2) — 실제 vCenter 로 "연결됨"/"전원을 켤 때 연결" 검증 + OS6 재빌드

아래 두 항목(EnsureConnectState 도입)은 커밋 시점엔 미검증이었고 OS6 도
소스만 반영돼 있었음(`bin_os6/` 는 이전 버전 그대로).

- 랩 vCenter(192.168.0.50)/ESXi(192.168.0.59)의 `192ev01` 로 왕복
  마이그레이션을 실행해 vCenter API 로 직접 재조회, 두 케이스 모두 확인:
  ...
  - 테스트 후 VM 을 원래 포트그룹·전원 꺼짐 상태로 정확히 복원.
- `build_os6.sh` 재실행해 `bin_os6/nm-*` 7종 전체 재생성(EnsureConnectState
  포함). go build/vet/test 통과, `nm-connect -h` 정상 동작 확인.
...
- 통합 스크립트(`change.sh port`)의 포트그룹 단계가 이 바이너리를 그대로 쓰므로,
  통합 실행에서도 동일하게 "연결됨" 까지 확인합니다. OS6 관리서버는
  `build_os6.sh` 로 `bin_os6/` 를 다시 빌드해야 반영됩니다.
