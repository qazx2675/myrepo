...
60초를 넘으면 화면이 **대체 화면 버퍼**(vim/htop/less 와 같은 방식)로 바뀌어
목록을 보여줍니다 — 매초 다시 그려도 터미널 스크롤백에는 쌓이지 않고, 나가면
실행 전 화면으로 그대로 돌아갑니다. 목록에는 **진행 중인 대상과 최근에 끝난
대상 최대 20개**만 나옵니다(대기 중이거나 오래전에 끝난 대상은 생략) — 대상이
수백~수천 대여도 매초 찍는 줄 수가 늘어나지 않게 하기 위함입니다. 전체
완료/실패 수는 위쪽 진행률 줄에서 항상 확인할 수 있고, 최종 결과는 실행이 끝나면
전체 목록이 빠짐없이 출력됩니다.
...
완료 12/40 (30%)
  ...
  ↑/↓ 로 선택, Enter 로 상세보기 (진행 중 + 최근 완료만 표시)

> web02                    IP 설정 적용  00:01:05
  web05                    연결 확인     00:00:03
