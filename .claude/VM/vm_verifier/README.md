...
- `targets.txt` (`-f`로 지정, 필수): 검증할 BM 접두어를 한 줄에 하나씩 (`#` 주석 가능).
  ...
  **VM 이름을 그대로 적어도 된다.** 아래처럼 적으면 실행 전에 접두어(`svr01`, `svr02`)로 자동 변환하고, 중복을 없앤 뒤 이름순으로 정렬해서 진행한다. 어차피 접두어마다 VM을 전부 자동 탐색하므로 결과는 접두어로 적었을 때와 같다.
  ```
  svr01ev01
  svr01ev02
  svr02ev01
  svr02ev02
  ```
  정리가 일어나면 시작할 때 `[INFO] 대상 목록 정리: 4줄 -> BM 접두어 2개 ...` 한 줄로 알려준다.
...
- vCenter 접속, DHCP 조회(DNS 포함), 검증까지 전부 goroutine 기반으로 병렬 처리된다 (worker pool로 동시 실행 수 제한, `-race` 디텍터로 데이터 레이스 없음 확인). 동시 실행 수는 `-concurrency`(기본 32)로 조정한다 — 대부분 DNS·파일 I/O 대기라 CPU 코어 수보다 크게 잡아도 된다.
- **조회 성능**: vCenter에서는 인벤토리 전체의 이름만 먼저 가볍게 훑고, 대상 접두어에 해당하는 VM에 대해서만 무거운 장치 정보(`config.hardware.device`)를 배치 조회한다. DHCP 대역 파일도 대역당 한 번만 읽어 캐시하고, DNS 조회는 그룹 구분 없이 한 번에 병렬 처리한다. 그래서 vCenter 인벤토리가 크거나 대상이 많아도 조회 시간이 급격히 늘지 않는다.
