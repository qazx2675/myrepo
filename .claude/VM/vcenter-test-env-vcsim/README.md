...
vcsim은 항상 **`127.0.0.1:54321` 고정 포트**로 뜹니다(`internal/builder/build.go`의 `Port` 상수). 이전에 백그라운드로 띄워두고 잊어버린 vc-test-env 프로세스 등이 이미 이 포트를 쓰고 있으면, vcsim을 기동하기 직전에 자동으로 감지해서 아래처럼 물어봅니다(`internal/portcheck`).
...
`y`를 입력하면 그 프로세스를 종료하고 이어서 진행하고, 그 외 입력이면 실행을 중단합니다(수동으로 정리 후 다시 시도).
...
- `-vc`에 이 도구 자신이 띄우는 vcsim 주소(`127.0.0.1:54321`)를 실수로 지정하면 지금은 즉시
  ...
  `~/.vc-test-env/recipes/127.0.0.1_54321.json`을 삭제하면 됩니다 — 실 vCenter(예: `192.168.0.50`)
  레시피는 건드리지 않습니다.
