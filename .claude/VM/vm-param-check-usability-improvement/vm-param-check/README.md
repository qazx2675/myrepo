...
또한 VM이 속한 vCenter 폴더 이름(예: `TST-CAE001-SAMP48c-QRST`)만으로 정상값(기대값) 옵션을 자동으로 채워 주는 **폴더명 기반 스펙 자동매칭**(`-specRoot`)을 지원합니다 — 매번 `-cpu`/`-cores`/`-numa`/... 를 손으로 붙이지 않아도 됩니다. 자세한 사용법은 아래 "2. 사용 방법 > 2-4. 폴더명 기반 스펙 자동매칭" 절을 참고하세요.
...
- **인터넷 불필요** — 의존성(`github.com/vmware/govmomi` 등)은 master에서는 `.claude/공통/govendor/govmomi-0.39.0`에 있고 `setup.sh`가 `vendor`로 링크합니다. 이 폴더만 떼어 폐쇄망으로 옮기려면 vendor가 실제 파일로 들어 있는 독립 브랜치 `vm-param-check-standalone`을 받으세요(상위 README 참고). (폴더명 자동매칭 기능도 기존에 이미 vendor에 있던 `property` 패키지만 추가로 쓰므로, `vendor/`를 새로 채울 필요가 없습니다.)
...
### 2-4. 폴더명 기반 스펙 자동매칭 (`-specRoot`) — 처음 쓰는 분도 바로 따라 할 수 있게

**무엇을 해결하나요?** 위 2-3절 예시처럼 매번 `-cpu`/`-cores`/`-numa`/`-mem`/`-disk`/`-shares-ev01`/`-ht` 같은 옵션을 손으로 붙이지 않아도, 체크하려는 VM이 어느 vCenter 폴더에 들어 있는지만 보고 알맞은 값을 자동으로 채워 주는 기능입니다. 현업에서는 VM 스펙별로 폴더명이 이미 규칙화되어 있다는 점(`TST-CAE001-SAMP48c-QRST`처럼)을 이용합니다.
...
shares-ev01=          # ratio 숫자(예: 4000) 또는 normal, 쉼표로 여러 개 나열 가능(예: 4000,normal)
...
ratio 숫자와 `normal`을 쉼표로 섞어서 여러 개 적을 수도 있습니다 — 이 경우 **목록 중 하나만 맞아도 OK**로 판정합니다(CPU/메모리 각각 독립적으로 판정하므로, 예를 들어 CPU는 ratio로 맞고 메모리는 normal로 맞아도 둘 다 OK가 됩니다). `shares-ev02`/`shares-ev03`도 동일하게 쉼표로 여러 개 허용합니다.

```
shares-ev01=4000,normal
```
...
1. **포트그룹명에서 유추**: VM에 붙은 네트워크 어댑터의 포트그룹 이름이 `<원래폴더명>-cae-옥텟-옥텟-옥텟-옥텟` 형태(예: `TST-CAE003-SAMP48c-QRST-cae-10-1-2-3`)면, 거기서 원래 폴더명을 복원해서 그 스펙으로 자동 진행합니다. 화면에는 이렇게 표시됩니다.
   ...
   ```
2. **자동으로 못 정하면 직접 물어봅니다**: 포트그룹으로도 못 찾으면(또는 후보가 여러 개면) 아래처럼 물어봅니다. `?`를 입력하면 `-specRoot` 아래 사용 가능한 스펙 목록을 보여 줍니다.
   ```
...
| `-shares-ev01 <N\|normal\|N,normal,...>` / `-shares-ev02` / `-shares-ev03` | 필수/옵션/옵션 | 기대값: CPU/메모리 Shares. ratio 숫자(예: `4000`) 또는 `normal`, 쉼표로 여러 개 나열 가능(예: `4000,normal`) — **목록 중 하나만 맞아도 OK**(CPU/메모리 각각 독립 판정). `normal`은 Level 자체가 `normal`인지, 숫자는 Level이 `custom`이고 ratio가 일치하는지를 봅니다. 세 플래그 모두 동일한 문법 |
...
- **그룹 동질성**: 같은 그룹(ev01끼리, ev02끼리, ev03끼리)의 **교정 대상끼리** vCPU/메모리/디스크/CPU Shares/HT가 같아야 함. **코어/소켓과 NUMA는 비교하지 않습니다** — 둘 다 `-fix`가 기대값으로 직접 고치는 항목이라 FAIL인 VM은 값이 다를 수밖에 없고, 비교하면 고치려는 바로 그 불일치 때문에 교정이 막히기 때문입니다. 반대로 메모리/디스크/Shares는 이 도구가 못 고치는(수동조치) 항목이라 다르면 진짜로 다른 스펙이므로 계속 비교합니다.
- **전원 OFF**: 교정 대상 VM이 전부 꺼져 있어야 함 (CPU 토폴로지를 하드웨어 레벨로 직접 바꾸기 때문)

> **ev01/ev02/ev03 짝(VM 대수)이 안 맞는 것은 게이트가 아닙니다.** 그룹별 VM 대수가 다르면 `[경고] 그룹별 VM 대수가 다릅니다 … 짝이 맞지 않지만 교정은 그대로 진행합니다`만 출력하고 계속 진행합니다(예전에는 이 경우 교정이 중단됐습니다). 대수는 PASS/FAIL과 무관하게 조회한 VM 전부로 세고, 접미사가 없는 VM("기타")은 세지 않습니다.
...
적용 전에 VM별로 "무엇을 어떤 값에서 어떤 값으로 바꿀지"를 전부 출력하고, 수동조치 대상 항목은 따로 개수만 요약해서 보여 줍니다. `-yes`가 없으면 `y`를 입력해야 다음 단계로 진행됩니다.
...
콘솔은 `[1]` 상세 섹션을 먼저 보여 주고, 맨 아래에 `[2] VM별 요약 표`가 나옵니다. 대수가 많으면 상세가 길어져서 요약이 스크롤 밖으로 밀리기 때문에, **마지막에 보이는 것이 요약**이 되도록 아래에 둡니다.

`-onlyFail`이면 **상세**는 FAIL이 있는 VM만, 그 안에서도 FAIL/설정없음/미지원 항목만 보여 줍니다. 하지만 **요약 표에는 PASS 서버도 전부 나옵니다** — 여기서까지 빠지면 "이 서버가 정상이었는지, 아예 검사가 안 된 건지"를 구분할 수 없기 때문입니다.
