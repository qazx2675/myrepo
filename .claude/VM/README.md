...
- `VM_setup` : VM 설정 적용(affinity/lpage/전원정책/태그/vSwitch/라이선스 할당/VM 생성 등)용 스크립트 모음 (V1, ev01~ev99 확장판은 `V2/VMsetup`)
- `V2` : VM 생성·설정(`VMsetup`, 호스트당 ev01~ev99)과 체크·교정(`vm-param-check-usability-improvement`)이 스펙 폴더 `SPEC_DIR`를 함께 쓰는 차세대 구성. `govendor/` 포함, V2 폴더만으로 오프라인 빌드
- `Network_Change_Integration_Script` : IP 변경 → LDAP 설정 → 포트그룹(VLAN) 이관을 한 번에 처리하는 통합 망변경 스크립트. 인시던트 자동 저장·역순 롤백 포함 (`vm-network-migration` 통합)
- `vCenter API IP 자동변경` : 네트워크로 접속할 수 없는 VM 의 IP 를 vCenter Guest Operations(VMware Tools) 경로로 재설정하는 도구(`vm-ip-change`)
- `인수인계` : 운영 도구 인수인계 문서 세트(.md 원본 + 오프라인 단일 HTML `VM_인수인계_핸드북.html`)
...
- `vm-network-migration` : (`Network_Change_Integration_Script`로 통합됨) VM 네트워크 포트그룹 일괄 이관 + 실패 시 자동 롤백 도구. 제어(`run.sh`)와 실행(단계별 Go 바이너리 6종)을 분리하고 전 과정을 워커풀로 병렬 처리
- `vm-param-check-usability-improvement` : VM 매개변수 체크/설정 통합 툴의 사용성·성능 개선 버전(옵션 자동화, 조회 속도 개선). 실제 도구는 이 폴더 아래 `vm-param-check/`
- `vm-param-setting-check` : (`vm-param-check`로 통합됨) vCenter의 VM들이 고성능 설정 기준(CPU/메모리/NUMA, vCPU affinity, Shares, 전원정책 등)을 만족하는지 자동 점검하는 도구
