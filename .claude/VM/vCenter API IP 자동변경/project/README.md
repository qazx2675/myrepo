...
**ev02 우선순위 확인 시험**: `-pair-mode ev02-only -busy-ev01 30` 처럼 주면(가장 흔한
실제 형태 — list.txt 엔 ev02 만 있고, 짝 ev01 은 부하 확인용으로만 조회됨) 그 비율만큼의
짝에서 `ev01` 이 "항상 과점유 중"으로 고정됩니다(부하가 바뀌지 않으므로 2분마다
재확인해도 절대 안 풀립니다 — 일부러 그렇게 만들었습니다: 실제로 부하가 안 내려가면
...
나머지는 정상 처리됐어야 합니다. `-pair-mode both` 로 바꿔 돌리면 짝 ev01 자신도
동시에 변경 대상이 되는 경우까지 같은 방식으로 확인할 수 있습니다.
