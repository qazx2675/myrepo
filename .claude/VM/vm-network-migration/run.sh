...
# 설정 파일 존재 확인 -> (필요 시) 빌드 -> 실행 순서로 진행합니다.
...
# 1) 설정 파일 확인 (인자로 다른 경로를 준 경우는 Go 쪽에서 다시 검사합니다)
if [[ "$*" != *"-vcenter-file"* && ! -f "$VCENTER_FILE" ]]; then
    echo "오류: vCenter 설정 파일이 없습니다. ($VCENTER_FILE)" >&2
    echo "      vcenter.txt.example 을 복사해서 채우세요:" >&2
    echo "      cp vcenter.txt.example vcenter.txt && chmod 600 vcenter.txt" >&2
    exit 1
fi
...
# 2) 빌드 (바이너리가 없거나 소스가 더 최신이면 다시 빌드)
NEED_BUILD=0
if [[ ! -x "$BIN" ]]; then
    NEED_BUILD=1
else
