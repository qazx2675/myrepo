...
| 남아 있는 이유 | V2에 대체 기능이 없습니다. `vm-param-check`는 호스트 전원정책을 **점검만** 하고 교정하지 않습니다 |

> **소스가 없는 유일한 사본입니다. 삭제·이동하지 마세요.** Go 소스를 확정하지 못해 재빌드할 수 없습니다.
> `VM_setup`의 나머지 도구는 모두 [10. V2](./10_V2.md)로 대체되었습니다.

**언제 실행하나요?** [전체 작업 흐름](./README.md)에서는 MAC 목록 추출(`mac_info`) 다음, 파워온 전 MAC 대조(`vm_verifier`) 전에 한 번 실행해 두는 것을 표준 순서로 둡니다 — 새 호스트의 전원 정책을 미리 맞춰 두면 이후 `vm-param-check`의 host 전원정책 점검이 항상 통과합니다. 이미 지나쳤거나 나중에 FAIL이 나오면 아래 명령으로 다시 실행해도 됩니다(멱등 — 이미 적용된 호스트는 건너뜀).
