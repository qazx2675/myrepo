...
실선은 `vm_setup.sh`가 실제로 호출하는 순서(vswitch → vm_create → affinity → lpage)이고, `mac_info`·`power_setting`·`license_assign`은 스크립트가 부르지 않는 **별도 후속 도구**입니다(1절 8·10번, [17번](./17_VM_setup_잔여도구.md)). 아래 흐름도는 이 도구들을 실무에서 쓰는 표준 위치 — VM 생성 직후(affinity·lpage 전) MAC 수집·전원정책 적용, 그리고 lpage 이후(신규 호스트만) 라이선스 할당 — 에 이어 붙인 것입니다.
...
flowchart TD
    ...
    I --> P["MAC 목록 추출 → DHCP 등록<br/>V2 mac_info 🟢 (1절 8번)"]:::cmd
    ...
    N --> M["[완료]"]:::safe
    M --> O["차이가 있으면 안내된 -fix 명령 (11번)<br/>무작위 VM 몇 대 직접 확인"]
    class G,Q gate
```
