...
| 폴더 | `.claude/VM/V2/` (서버에는 `/home/`에 네 폴더를 나란히 배치) |
...
| 인증 | `VC_PASSWORD` (계정은 `vm_setup.sh -i`, 개별 도구 `-id`) |
...
# 3) 신규 VM 생성·설정 — 실제 실행 (vCenter 는 vcenter.txt 목록에서 번호로 선택, Enter = 직전 실행 vCenter)
./vm_setup.sh -u <작업이름>

# 4) vCenter 를 직접 지정해서 실행
./vm_setup.sh -u <작업이름> -v <vCenter IP> -i <vCenter 계정>

# 5) vswitch_<작업이름>.txt 의 포트그룹 칸에 IP 를 적어 둔 경우 — 실행 전에 이름 형식으로 변환
bash vswitch_pgname.sh ../SPEC_DIR/vswitch_<작업이름>.txt

# 6) 생성 후 설정 점검 (같은 SPEC_DIR 사용) → 11번 문서
...
```mermaid
flowchart TD
    A["준비<br/>VMsetup/작업이름.txt (BM 목록)<br/>SPEC_DIR/vswitch_작업이름.txt (BM 포트그룹 VLAN)<br/>vcenter.txt"] --> B{"포트그룹 칸이 IP?"}
    B -- 예 --> B1["vswitch_pgname.sh<br/>IP → 폴더명-cae-a-b-c-0, 원본 .bak"] --> C
    B -- 아니오 --> C["./vm_setup.sh -u 작업이름"]
    C --> D["1. 스펙 할당<br/>포트그룹 폴더명 → SPEC_DIR 자동 매칭"]
    D --> D1{"표 확인 y/n"}
    D1 -- n / 자동 실패 --> D2["번호로 선택<br/>0 → vim 으로 새 스펙 + ev별 affinity"] --> E
    D1 -- y --> E["2. 포트그룹 할당 (네트워크 어댑터 1)"]
    E --> E1{"표 확인 y/n"}
    E1 -- n / 자동 실패 --> E2["번호로 선택 / 0 → vim"] --> F
    E1 -- y --> F["3. vCenter 선택 (-v 없을 때)<br/>Enter = 직전 실행"]
    F --> G{"4. 실행 계획 확인 y/n<br/>-n 이면 여기서 종료"}
    G -- n --> Z["종료 (변경 없음)"]
    G -- y --> H["vswitch_setting (호스트 병렬)"]
    H --> I["vm_create (스펙별)"]
    I --> J["affinity_setting"]
    J --> K["lpage_setting"]
    K --> L{"단계 실패?"}
    L -- 예 --> L1["그 단계에서 멈춤, [완료] 없음<br/>원인 수정 후 재실행 → 이미 있는 포트그룹/VM 은 건너뜀"] --> C
    L -- 아니오 --> M["[완료]"]
    M --> N["vm-param-check -specRoot=../../SPEC_DIR (11번)"]
    N --> O["무작위 VM 몇 대 직접 확인"]
```
...
cp V2/vcenter.txt.example /home/vcenter.txt        # vCenter 주소로 수정
cd /home && bash setup.sh                           # Go 1.26.5 이상, 11종 오프라인 빌드
```

`vm_setup.sh`는 실행 파일이 없으면 스스로 빌드합니다.
...
| `SPEC_DIR/vswitch_<작업이름>.txt` | `BM  포트그룹  VLAN` (BM당 여러 줄 가능 → 포트그룹 여러 개) | `esxi-node-001.domain  TST-CAE001-SAMP48c-QRST-cae-10-1-2-3  100` |
| `/home/vcenter.txt` | vCenter 주소 (없으면 vm-param-check 폴더의 `vcenter.txt`) | `192.168.0.50` |
...
두 파일(`<작업이름>.txt`, `vswitch_<작업이름>.txt`)의 BM 표기는 같게 적습니다. 포트그룹 이름을 `<CAE폴더명>-cae-a-b-c-d`로 지으면 그 폴더명으로 스펙이 자동 할당됩니다.
...
### 5-1. `vm_setup.sh` (소스: `getopts "u:v:i:s:w:c:nh"`)

| 옵션 | 기본값 | 필수 | 설명 |
|---|---|---|---|
| `-u <작업이름>` | — | ✅ | `<작업이름>.txt`, `SPEC_DIR/vswitch_<작업이름>.txt`를 찾는 이름. 영문/숫자/`._-`만 |
| `-n` | — | | 스펙·포트그룹 할당과 실행 계획까지만 보여 주고 종료 (vCenter 변경 없음) |
| `-v <ip>` | 환경변수 `VC_IP` | | vCenter IP. 없으면 `vcenter.txt` 목록에서 번호 선택 (`0` 직접 입력, Enter 직전 실행) |
| `-i <계정>` | `administrator@vsphere.local` | | vCenter 계정 |
| `-s <dir>` | `../SPEC_DIR` | | SPEC_DIR 경로 |
...
| 증상 | 원인 | 해결 |
|---|---|---|
| `SPEC_DIR 를 찾을 수 없습니다` | `/home/SPEC_DIR`가 없음 | 네 폴더를 `/home`에 나란히 배치하거나 `-s` 지정 |
| 스펙이 자동 할당되지 않고 이유가 출력됨 | 스펙에 `affinity-evNN`이 없는 ev가 있음 | 스펙에 ev마다 affinity 파일 지정 |
...
| `govendor/ 가 없습니다` | `/home/govendor` 누락 | V2 폴더 전체 복사 |
| `VC_PASSWORD 환경 변수가 설정되지 않았습니다` | 개별 도구 실행 시 비밀번호 없음 | `export VC_PASSWORD` |
...
│   ├── user.txt.example
│   ├── CHANGELOG.md
│   ├── run_<user>/              # (자동 생성) 입력 사본, last_vcenter
