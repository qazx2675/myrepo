...
//   - 그룹 동질성: ev01끼리, ev02끼리, ... ev99끼리 스펙이 같아야 함(교정 대상끼리 비교)
//   - 전원 OFF: 대상 VM이 전부 꺼져 있어야 함 (CPU 토폴로지를 직접 바꾸기 때문)
//
// ev01~ev99 그룹 간 VM 대수가 맞는지(짝이 맞는지)는 게이트가 아니다. 다르면
...
// GroupCountWarning은 ev01~ev99 그룹이 둘 이상 섞여 있을 때 각 그룹의 VM 대수가
