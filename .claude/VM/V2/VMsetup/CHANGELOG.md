...
- **모니터 켜지는 조건**: 표준입력·표준에러가 터미널이고 `VMSETUP_MONITOR=0` 이 아닐 때. 파이프 입력·nohup·cron 은 예전처럼 줄줄이 출력한다(같은 단계 목록·같은 단계 함수 — 로직은 한 벌).
- **로그 분리**: 단계별 `run_<user>/logs/NN_<단계>.log`, 전체 `run_<user>/run.log`, 실행 계획 `run_<user>/plan.txt`(모니터 "작업 시작" 줄에서 Enter). 실행 계획을 파이프(`tee`)로 저장하려고 계획 출력을 `print_plan` 함수로 묶었다(출력 내용은 그대로 + `라이선스 할당` 줄).
- **`license_assign` 편입**: 스펙별 단계가 모두 끝난 뒤 BM 전체 대상으로 1회(스펙 체크 전). 선택 입력이 필요하므로 모니터를 잠시 닫고 전경에서 실행 → 끝나면 복귀. `q`(도구 종료코드 0)는 로그의 "작업을 중단합니다" 줄로 `[취소]` 표시 후 다음 단계 계속. 표준입력이 터미널이 아니면 남은 입력 뒤에 `q`를 붙여 넘긴다(입력이 끝나면 도구가 빈 입력을 "잘못된 번호"로 보고 끝없이 다시 묻는 것 방지). `ensure_bin` 목록에 추가. Go 도구는 한 줄도 바꾸지 않았다(OS6 재빌드 불필요).
- **구현 메모**: 백그라운드 단계는 `set -m` 으로 자기 프로세스 그룹에서 돌린다 — bash 의 `read -n` 이 키를 기다리는 동안 ISIG 를 다시 켜서 Ctrl+C 가 SIGINT 로 오고, INT trap 때문에 서브셸의 SIGINT 가 기본 동작으로 돌아가 한 번 누르면 도구가 죽는 것을 시험에서 확인해 고쳤다. 화면은 줄마다 `CSI 2K` 로 지우고 그린다(열 이동만 하면 이전 화면 글자가 남는 것 확인). bash 4.1 호환(`read -t` 소수 초, `tput`, 방향키 ESC 시퀀스 조립).
- **영향 범위**: `VMsetup/vm_setup.sh`, `VMsetup/README.md`, `VMsetup/workflow.mmd`(license_assign 노드·모니터 표시 — `workflow.svg` 는 다시 렌더링하지 않음), `VM-10대-확장-및-SPEC_DIR-공유/검증/saccae/사용법.txt`.
- **검증** (.58 Rocky bash 4.4 + vcsim, CentOS 6 컨테이너 bash 4.1.2): python3 pty + pyte(가상 터미널)로 키를 흘려 넣고 화면을 캡처.
  - `-u lsh`(BM 40대, VM 240대): `[대기]→[진행]→[완료]`, 경과 시간 증가, ↑ Enter 로 이전 단계 로그·PgUp, 진행 중 단계 로그가 따라감, 라이선스에서 모니터가 닫히고 선택 화면 → `0`(vcsim 은 Evaluation Mode 총량 0 뿐이라 할당 안 되고 다시 물음) → `q` → 모니터 복귀 `[취소]`, 끝나면 커서 `[종료]`, Enter 1번 "한 번 더 Enter" 안내, 2번 종료 → 최종 요약, 종료코드 0.
  - `-u fail_nohost`: `포트그룹 생성 [실패] 성공 1 / 스킵 0 / 실패 1`, 나머지 `[중단]`, 커서가 실패 단계 → Enter 로 원인 로그 → `[종료]` Enter 두 번 → 요약에 로그 끝 5줄, 종료코드 1.
  - Ctrl+C: `vm_create` 진행 중 1·2번은 안내만(도구 계속 실행 — 프로세스 확인), 3번째에 `vm_create` 종료·터미널 복원·`[중단]` 요약, 종료코드 130, 남은 프로세스 없음.
  - 파이프 입력(`printf ... | vm_setup.sh -u lsh`)과 터미널 + `VMSETUP_MONITOR=0`: 모니터 없이 예전처럼 출력, 라이선스는 `q`로 취소, 종료코드 0.
  - OS6 컨테이너(`bin_os6` 실행파일, bash 4.1.2)에서 `-u lsh` 모니터 전 과정 동일하게 동작, 종료코드 0.
  - 회귀: `검증/vmsetup_test.sh` 는 이번 변경 전(`09dd730`)부터 깨져 있었다(0/48 — `vswitch_<user>.txt` 를 옛 위치 `SPEC_DIR` 에 만듦). 경로만 고친 임시 사본으로 .58 `/root/v2work/V2`(브랜치로 갱신, 이전 것은 `V2.bak-20260926`)에서 돌리면 변경 전·후 **PASS/FAIL 목록이 동일**(24/24 — 실패 24건은 둘 다 시험용 vcsim 에 `HostPowerSystem` 이 없어 `power_setting` 이 실패하는 것).
  - 미확인: 실제 라이선스 할당 성공 경로(vcsim 에 쓸 수 있는 라이선스가 없음) — 실제 vCenter 에서 확인 필요.
