...
6~7번(실행 확인 y 이후)은 터미널에서 실행하면 도구 출력을 흘려보내지 않고 **단계별 요약 화면**으로 보여줍니다(대체 화면이라 끝나면 원래 화면으로 돌아가고, 최종 요약만 남습니다).
...
| `[종료]` 줄에서 Enter 두 번 | 모니터 종료 → 최종 요약 출력. 다 끝나면 커서가 `[종료]`에 있으므로 Enter 두 번이면 바로 끝남 |
| Ctrl+C 3번(5초 안) | 실행 중인 도구를 끝내고 종료(종료코드 130). 1·2번째는 안내만 |

- 상태: `[대기]` `[진행]` `[완료]` `[실패]` `[건너뜀]` `[중단]`(앞 단계 실패/Ctrl+C로 실행 안 함) `[취소]`(라이선스 `q`).
- **라이선스 할당**은 모니터를 잠시 닫고 선택 화면을 평소처럼 보여준 뒤 모니터로 돌아옵니다. 여기서 Ctrl+C 를 누르면 라이선스 선택만 `[취소]`되고 다음 단계로 갑니다.
- 단계가 실패하면 그 단계 `[실패]`, 남은 단계 `[중단]`, 커서가 실패 단계에 놓입니다(Enter 로 원인 확인). 종료코드 1.
