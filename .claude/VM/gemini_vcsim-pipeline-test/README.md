...
모든 명령어는 Rocky Linux 서버의 프로젝트 디렉토리(`/root/myrepo/.claude/VM/gemini_vcsim-pipeline-test`)에서 수행합니다.

### 실행 명령어

```bash
cd "/root/myrepo/.claude/VM/gemini_vcsim-pipeline-test"
...
echo 'cd "/root/myrepo/.claude/VM/gemini_vcsim-pipeline-test" && go test -mod=vendor -v -timeout 180s ./...' >> test-vcsim
chmod +x test-vcsim
