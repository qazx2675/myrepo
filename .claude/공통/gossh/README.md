...
### 5.11 표준출력/표준에러 분리 (6번째 업데이트)

`gossh -w hosts.txt "uptime" > res`처럼 **표준출력만** 파일로 리다이렉션했을 때, 실제 명령
실행 결과(성공한 호스트의 출력)만 `res`에 남고, 그 외의 모든 것 — 접속 실패/Refused, 명령어
오타로 인한 에러("command not found" 등), `[안전장치]`/위험 작업 경고, 작업 요약 블록 —는
표준에러로 나가서 `res`에 안 남는다. 일반적인 CLI 도구의 stdout(데이터)/stderr(상태·진단)
구분 관례를 그대로 맞춘 것이다. `gossh`/`pdsh` 이름 둘 다 동일하게 적용된다.

```bash
./gossh -w hosts.txt "uptime" > res
# hosts.txt의 서버가 전부 접속 불가면: res는 완전히 비어있음(에러/요약은 화면(stderr)에만 표시)
# 접속되는 서버가 있으면: res에는 그 서버들의 uptime 출력만 남음

./gossh -w hosts.txt "uptime" > res 2>err
# 접속불가/오타/요약 등 상태성 메시지를 따로 보고 싶으면 err로 받으면 됨
```

`-b` 그룹 출력도 동일한 원칙을 따른다 — 결과가 완전히 같은 성공 사례만 묶여서 표준출력에 남고,
명령어 실행 에러가 난 호스트는 그룹으로 묶이지 않고 즉시 표준에러로 나간다.

### 5.12 진행률 표시 (7번째 업데이트)

작업 대상이 많을 때, 지금까지 몇 대가 끝났는지 1초 간격으로 표준에러에 보여준다(`\r`로 같은 줄을
덮어씀). 데이터가 아니라 상태 표시라 표준에러로 나가므로 `> res` 리다이렉션에는 안 섞인다.

```
진행: 1230/3000 (41%)
```

**실행 파일 이름이 `pdsh`면 이 진행률 표시가 아예 안 뜬다**(pdsh에 없는 gossh 전용 출력이 섞이면
안 되기 때문 — `[안전장치]` 메시지와 같은 원칙). 카운트는 접속 성공/실패와 무관하게 호스트별
처리가 "끝났을 때"(성공/실패/타임아웃 등 무엇이든) 올라간다.
