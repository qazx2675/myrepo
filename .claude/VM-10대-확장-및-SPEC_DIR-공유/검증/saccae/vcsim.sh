...
start() {
  ...
  TMPDIR="$DATA" setsid nohup "$BIN" -dc 1 -cluster 0 -host 0 -fqdnHosts "${hosts%,}" -staticPower -addr "$ADDR" > "$LOG" 2>&1 < /dev/null &
  echo $! > "$PID"
