...
vcsim은 항상 **`127.0.0.1:54321` 고정 포트**로 뜬다(`internal/builder/build.go`의 `Port` 상수) — 재시작해도 매번 같은 주소라 `vcenter.txt` 같은 걸 다시 고칠 필요가 없다. 이미 이 포트를 다른 프로세스가 쓰고 있으면 기동에 실패하니, 그럴 땐 그 프로세스(보통 이전에 떠 있던 vc-test-env)를 먼저 종료할 것.
...
PowerCLI에서 직접 확인하고 싶으면 아무 터미널에서:
...
- VM 네트워크 어댑터: 포트그룹 이름 + 커넥트/디스커넥트 상태를 실제 디바이스로 재현(호스트에 포트그룹을 먼저 만들고 VM 생성 시 어댑터를 붙임) — `vm-param-check`의 네트워크 체크를 vcsim에서도 그대로 테스트할 수 있음
- 호스트: 전원정책(조회만 — vcsim 재현은 TODO, `internal/builder/build.go` 참고)
...
- `build`: vcsim에 동일 구조 재생성 성공 — 확인 완료. (아래 한계 참고)
...
- **어댑터 커넥트 상태가 VM마다 100% 정확히 재현되진 않음**: 레시피의 `connected` 값을 그대로 넘기는데도 일부 VM(`vcenter`)에서 실 vCenter와 다르게 나오는 사례를 확인함 — 원인은 더 파봐야 함. 포트그룹 이름 자체는 항상 정확함.
