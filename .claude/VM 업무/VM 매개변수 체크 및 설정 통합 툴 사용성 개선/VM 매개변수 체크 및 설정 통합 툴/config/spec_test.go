...
func TestNormalizeFolderName(t *testing.T) {
	tests := []struct {
		...
		{"실예시1", "RND-CAE001-LICE48c-TCAD", "RND-CAE-LICE48c-TCAD", true},
		{"실예시2", "MEM-CAE001-CPSR16c-ECAD", "MEM-CAE-CPSR16c-ECAD", true},
		{"실예시3 (+ 포함)", "MEM-CAE001-PICE16c-ECAD+VWP", "MEM-CAE-PICE16c-ECAD+VWP", true},
		// 차수만 다른 건 같은 스펙으로 모여야 한다.
		{"차수 다름", "RND-CAE003-LICE48c-TCAD", "RND-CAE-LICE48c-TCAD", true},
		{"차수 자릿수 다름", "RND-CAE12-LICE48c-TCAD", "RND-CAE-LICE48c-TCAD", true},
		// 규칙에서 벗어나면 매칭 대상이 아니다.
		{"레코드 3개", "RND-CAE001-TCAD", "", false},
		{"레코드 5개", "RND-CAE001-LICE48c-TCAD-X", "", false},
		{"2번째가 CAE 아님", "RND-XYZ001-LICE48c-TCAD", "", false},
		{"2번째에 숫자 없음", "RND-CAE-LICE48c-TCAD", "", false},
		{"Task 폴더", "Task", "", false},
	}
...
func TestNormalizeFolderNameDistinct(t *testing.T) {
	samples := []string{
		"RND-CAE001-LICE48c-TCAD",
		"MEM-CAE001-CPSR16c-ECAD",
		"MEM-CAE001-PICE16c-ECAD+VWP",
	}
...
func TestFindSpec(t *testing.T) {
	root := t.TempDir()
	specDir := filepath.Join(root, "RND-CAE001-LICE48c-TCAD")
	if err := os.MkdirAll(specDir, 0o755); err != nil {
		t.Fatal(err)
	}
	specFile := filepath.Join(specDir, "RND-CAE001-LICE48c-TCAD_spec.txt")
	if err := os.WriteFile(specFile, []byte("ht=on --cores=20\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	// 관계없는 스펙도 하나 둬서 엉뚱한 게 잡히지 않는지 본다.
	other := filepath.Join(root, "MEM-CAE001-CPSR16c-ECAD")
	if err := os.MkdirAll(other, 0o755); err != nil {
		t.Fatal(err)
	}

	// 차수가 달라도(CAE007) 같은 스펙으로 찾아야 한다.
	m, err := FindSpec(root, "RND-CAE007-LICE48c-TCAD")
	if err != nil {
