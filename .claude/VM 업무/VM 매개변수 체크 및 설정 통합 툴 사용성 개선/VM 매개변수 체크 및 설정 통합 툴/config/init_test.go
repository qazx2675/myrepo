...
func TestInitFolderBlank(t *testing.T) {
	...
	specFile, err := InitFolder(root, "RND-CAE001-LICE48c-TCAD", "")
	if err != nil {
...
func TestInitFolderDuplicateRejected(t *testing.T) {
	root := t.TempDir()
	if _, err := InitFolder(root, "RND-CAE001-LICE48c-TCAD", ""); err != nil {
		t.Fatal(err)
	}
	// 차수만 다른 이름으로 다시 만들면(같은 스펙으로 매칭됨) 막혀야 한다.
	if _, err := InitFolder(root, "RND-CAE009-LICE48c-TCAD", ""); err == nil {
...
func TestInitFolderWithTemplate(t *testing.T) {
	...
	srcDir := filepath.Join(root, "MEM-CAE001-CPSR16c-ECAD")
	...
	if err := os.WriteFile(filepath.Join(srcDir, "MEM-CAE001-CPSR16c-ECAD_spec.txt"), []byte(srcSpec), 0o644); err != nil {
		t.Fatal(err)
	}

	specFile, err := InitFolder(root, "MEM-CAE002-CPSR32c-ECAD", "MEM-CAE009-CPSR16c-ECAD")
	if err != nil {
		t.Fatalf("InitFolder(템플릿 사용) 실패: %v", err)
	}

	m, err := FindSpec(root, "MEM-CAE005-CPSR32c-ECAD")
	if err != nil {
