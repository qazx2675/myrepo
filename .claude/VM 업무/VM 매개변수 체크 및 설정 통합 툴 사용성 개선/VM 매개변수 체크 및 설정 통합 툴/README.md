...
또한 VM이 속한 vCenter 폴더 이름(예: `RND-CAE001-LICE48c-TCAD`)만으로 정상값(기대값) 옵션을 자동으로 채워주는 **폴더명 기반 스펙 자동매칭**(`-specRoot`)을 지원합니다 — 매번 `-cpu`/`-cores`/`-numa`/... 를 손으로 붙이지 않아도 됩니다. 자세한 사용법은 아래 "2. 사용 방법 > 2-4. 폴더명 기반 스펙 자동매칭" 절을 참고하세요.
...
cd myrepo
tar czf vm-param-check.tar.gz ".claude/VM 업무/VM 매개변수 체크 및 설정 통합 툴 사용성 개선/VM 매개변수 체크 및 설정 통합 툴"
```

### 빌드 (인터넷 여부와 무관 — vendor/ 포함되어 있음)
```bash
cd ".claude/VM 업무/VM 매개변수 체크 및 설정 통합 툴 사용성 개선/VM 매개변수 체크 및 설정 통합 툴"
bash setup.sh
```
...
**무엇을 해결하나요?** 위 2-3절 예시처럼 매번 `-cpu`/`-cores`/`-numa`/`-mem`/`-disk`/`-shares-ev01`/`-ht` 같은 옵션을 손으로 붙이지 않아도, 체크하려는 VM이 어느 vCenter 폴더에 들어있는지만 보고 알맞은 값을 자동으로 채워주는 기능입니다. 현업에서는 VM 스펙별로 폴더명이 이미 규칙화되어 있다는 점(`RND-CAE001-LICE48c-TCAD`처럼)을 이용합니다.
...
**폴더명 규칙**: vCenter에서 VM이 들어있는 폴더 이름(예: `RND-CAE003-LICE48c-TCAD`)을 하이픈(`-`)으로 나눴을 때, **1·3·4번째 조각은 그대로 같아야 하고, 2번째 조각은 `CAE` 뒤의 숫자(차수)만 다르면 같은 스펙으로 인정**됩니다. 즉 `RND-CAE001-LICE48c-TCAD`와 `RND-CAE003-LICE48c-TCAD`는 (차수만 다르므로) **같은 스펙**으로 취급되고, `RND-CAE001-LICE48c-TCAD`와 `MEM-CAE001-LICE48c-TCAD`는 (1번째 조각이 다르므로) **다른 스펙**입니다.
...
1. **포트그룹명에서 유추**: VM에 붙은 네트워크 어댑터의 포트그룹 이름이 `<원래폴더명>-cae-옥텟-옥텟-옥텟-옥텟` 형태(예: `RND-CAE003-LICE48c-TCAD-cae-10-111-111-0`)면, 거기서 원래 폴더명을 복원해서 그 스펙으로 자동 진행합니다. 화면에는 이렇게 표시됩니다.
   ```
...
| `-initFolder <폴더명>` | (없음) | **vCenter에 연결하지 않고** `-specRoot` 아래에 이 이름의 스펙 디렉터리와 `<이름>_spec.txt` 스캐폴드를 만들고 종료한다. 이름은 CAE 폴더 규칙(하이픈 4개 레코드, 2번째가 `CAE<숫자>`)을 따라야 한다. 이미 같은 스펙(차수만 달라도)이 있으면 덮어쓰지 않고 에러로 중단 |
...
### 4.2 콘솔 출력 / 출력 파일 형식
콘솔은 항상 `[1] VM별 요약 표`를 먼저 보여주고 `[2]` 상세 섹션이 이어집니다. `-onlyFail`이면 FAIL이 있는 VM만, 그 안에서도 FAIL/설정없음/미지원 항목만 보여줍니다.

CSV 2종(상세/요약)이 생성됩니다.
- **상세 CSV**: `VM명, 소스, 항목Key, 기대값, 실제값, 결과, 비고` — OK 포함, 필터링 없음(`-onlyFail` 지정 시엔 콘솔과 동일하게 PASS VM 제외)
...
#### 매칭 규칙
폴더명을 하이픈(`-`)으로 나눴을 때 정확히 4개 조각이어야 하고, **1·3·4번째 조각은 완전히 같아야, 2번째 조각은 `CAE` 뒤 숫자(차수)만 정규화(무시)하고 비교**합니다. 예: `RND-CAE001-LICE48c-TCAD`와 `RND-CAE003-LICE48c-TCAD`는 같은 스펙, `RND-CAE001-LICE48c-TCAD`와 `MEM-CAE001-LICE48c-TCAD`는 다른 스펙입니다. 이 규칙에 안 맞는 폴더명(4개 조각이 아니거나 2번째가 `CAE<숫자>` 형태가 아닌 경우, 예: `Task`)은 아래 "Task 폴더 예외 처리"로 넘어갑니다.
...
### 4.7 기존 개별 도구와의 관계
저장소의 `VM 매개변수설정체크/`(체크 전용)와 `VM 매개변수설정체크/FAIL기반 매개변수 수정/`(레거시 외부 도구 오케스트레이션 방식)는 삭제하지 않고 그대로 남겨뒀습니다. 이 폴더는 그 둘을 대체하는 통합본입니다 — 새로 시작하는 경우 이 폴더만 쓰면 됩니다.
