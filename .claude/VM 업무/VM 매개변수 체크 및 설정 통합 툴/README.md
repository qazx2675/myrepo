...
- **인터넷 불필요** — `vendor/`에 의존성(`github.com/vmware/govmomi` 등)이 전부 포함되어 있어서 이 폴더 하나만 옮기면 폐쇄망에서도 바로 빌드됩니다.
...
결과 값은 5가지: `OK`(일치) / `FAIL`(설정은 있으나 다름) / `설정없음`(아예 미설정) / `미지원`(대상이 vcsim 시뮬레이터일 때만 나타남 — 실제 vCenter엔 있지만 vcsim이 구현하지 않아 값 자체를 조회할 수 없는 필드. 아래 13장 "알려진 한계" 참고) / `정보`(판정 없는 정보성 항목).
