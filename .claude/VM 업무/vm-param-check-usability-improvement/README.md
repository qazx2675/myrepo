...
**스펙이 아직 없다면** (`-specRoot` 아래 디렉터리가 비어있다면) 먼저 만들어야 합니다 — VM이 속한 vCenter 폴더 이름(예: `RND-CAE001-LICE48c-TCAD`)과 똑같은 이름으로:
...
# 9) 생성된 ./SPEC_DIR/RND-CAE001-LICE48c-TCAD/RND-CAE001-LICE48c-TCAD_spec.txt 파일을
...
| 문제 | 해결 |
|---|---|
| 체크할 때마다 `-cpu`/`-cores`/`-numa`/`-mem`/`-disk`/`-shares-ev01`/`-ht`를 손으로 입력해야 함 | VM이 속한 vCenter 인벤토리 폴더 이름(예: `RND-CAE001-LICE48c-TCAD`)만으로 스펙 파일을 자동으로 찾아 옵션을 채우는 **`-specRoot` 폴더명 기반 스펙 자동매칭** 추가 |
