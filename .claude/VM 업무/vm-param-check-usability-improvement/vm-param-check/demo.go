...
func runDemo(out, user string, onlyFail, noColor bool) {
	...
	for _, vm := range vms {
		...
		findings = append(findings, checker.CheckHardware(vm, checker.CPUExpect{Base: demoExpectCPU}, checker.MemExpect{Base: demoExpectMemGB}, checker.DiskExpect{Base: demoExpectDiskGB}, shares, group, singleVMMode, false)...)
