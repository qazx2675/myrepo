...
func printSummaryTable(w io.Writer, statuses []VMStatus, color bool) {
	...
	for _, s := range statuses {
		...
		unsupportedPlain := fmt.Sprintf("%8d", s.Unsupported)
		unsupportedDisp := colorize(unsupportedPlain, ansiBlue, color && s.Unsupported > 0)
...
// onlyProblems=true면 FAIL/설정없음/미지원만, false면 OK/정보까지 전부 출력한다.
func printItems(w io.Writer, items []model.Finding, color bool, onlyProblems bool) {
	sort.SliceStable(items, func(i, j int) bool { return items[i].Key < items[j].Key })
	for _, f := range items {
		if onlyProblems && f.Result != "FAIL" && f.Result != "설정없음" && f.Result != "미지원" {
			continue
		}
		var tag string
		switch f.Result {
		...
		case "미지원":
			tag = colorize("[미지원]      ", ansiBlue, color)
		case "FAIL":
		...
		case "미지원":
			fmt.Fprintf(w, "  %s%s%s: 가상환경에서 지원하지 않는 필드\n", tag, f.Key, src)
		case "정보":
