...
- `-monitor` 플래그 추가(기본 꺼짐 — 끄면 화면은 기존과 동일). 켜면 60초가 지나도
  안 끝날 때 `/dev/tty` 에 호스트별 진행 화면(페이지 보기, ↑/↓·←/→·Enter 상세)을
  띄우고, Ctrl+C 는 5초 안에 3번 눌러야 종료(gossh 정리 후 종료코드 130).
  `internal/monitor` 는 ip_change 와 같은 파일. NOCHANGE 는 "완료(변경없음)" 표시.
- 적용 시 **사이트별 gossh 를 순서대로 → 동시에** 실행. 한 사이트에 CPU 를 거의 못
  받는 VM 이 있으면(몇 시간~며칠) 다른 사이트가 시작조차 못 하던 문제. 사이트
  스크립트를 전부 먼저 만든 뒤 실행하므로 한 사이트라도 생성에 실패하면 아무것도
  건드리지 않음(예전엔 앞 사이트는 이미 적용된 뒤 멈췄음). 결과 출력 순서·형식·
  종료코드는 기존과 같음(`[사이트] N대 → gossh 전송` 줄만 결과보다 앞에 모여 찍힘).
  동시 SSH 세션 수는 사이트 수 × `-c` 가 됨.
- `internal/remote.Run`: 줄 단위 전달(ip_change 와 동일).
...
- 직전 항목에서 `cmd.Stderr = nil` 로 gossh 의 표준에러를 아예 버렸는데, 이
  ...
  (표준에러)까지 같이 사라져 원인을 알 수 없게 됨. `Run()` 이 표준출력·
  표준에러를 각각 별도 버퍼로 받아 **파싱은 표준출력만** 쓰되, **화면
  표시용 raw 에는 표준에러도 붙이도록** 수정.
