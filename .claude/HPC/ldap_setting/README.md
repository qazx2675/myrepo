...
### 2.1-a 대화형으로 실행하기 — `scripts/run.sh`

명령행 플래그를 외우지 않고 메뉴로 계정·인프라를 고르며 실행하고 싶을 때 씁니다.
`deploy_ldap.sh` 와 달리 **검증(gossh 로 ldap_check.sh 실행)은 하지 않고 적용까지만** 합니다.
...
```
=== LDAP 설정 자동화 — 실행 계정 선택 ===
작업을 수행할 계정을 선택하십시오:
선택 (번호 또는 계정명 직접 입력): admin1
...
########## DRY-RUN — 무엇이 바뀔지 먼저 확인합니다 ##########
...
위 DRY-RUN 결과대로 실제로 적용하시겠습니까?
계속하려면 정확히 'yes' 를 입력하십시오: yes

########## 실제 적용 ##########
...
```

- **계정 선택 함수(`select_user_context`)는 계정 목록이 비어 있는 골격만** 있습니다.
  아무 값이나 입력하면 그 값이 그대로 계정명이 되고, 계정명과 같은 이름의
...
- 사이트별 처리는 선택하지 않습니다. 대상 파일의 `site` 컬럼을 보고 **엔진이 알아서**
  사이트별로 나눠 적용합니다.
- **DRY-RUN → `yes` 확인 → 실제 적용** 순서가 고정되어 있습니다. `yes` 가 아닌 어떤
  입력도 취소로 처리하고 아무것도 바꾸지 않습니다.
- 검증과 되돌리기는 마지막에 안내 문구로 알려줄 뿐 자동 실행하지 않습니다. 직접
  `./deploy_ldap.sh -infra <이름> -check-only` 또는 `bin/ldap-config-engine ... -rollback` 을 쓰십시오.
