...
### 1.2 전역 명령어로 사용하기 (선택 사항)
빌드된 실행 파일이나 스크립트를 PATH 환경 변수에 포함된 디렉터리로 이동하거나, 실행 파일이 있는 경로를 PATH에 추가하면 어디서든 명령어처럼 사용할 수 있습니다.

예시 (스크립트를 `/usr/local/bin`으로 복사):
```bash
sudo cp os_check_final_annotated.sh /usr/local/bin/
...
├── README.md                      # 이 문서
├── PLAN.md                        # 개발/변경 계획 메모
└── os_check_final_annotated.sh    # OS 점검 → 환경설정 적용 → 재점검을 처리하는 스크립트 본체
```
