...
### Added
- A 재조사 단계에 `========== 4) A 서버 재조사 ==========` 배너와 판정값
  (`enabled` / `host` / 대상 수 / 자산행 수)을 **항상** 출력. 어느 분기로 가든 사유가 보인다.
- 실행 끝에 `A 재조사 반영됨` / `A 재조사 미반영 — ...` 요약 한 줄 추가.
- `--debug` 시 conf 의 `[server_a]` 섹션 **원문**을 그대로 출력 (스크립트가 읽은 값과 대조용).
- A 가 정상 종료했는데 `result_*.tsv` 가 없으면 경고 (기존엔 "0개 수집" 으로 조용히 넘어감).
