...
| **6** | [S4] `pxe` 4개 옵션 조합 + 호스트 수 리포트 | ✅ 완료 (2026-08-19) — `-infra`/`-os`/`-boot`/`-splunk` 4개 옵션(각각 번호/값/생략) 조합 실행, 성공 시 "총 N대의 호스트가 등록 완료되었습니다." 출력, 실패 시 stdout 마지막 30줄. 스텁 서버로 전체 성공(+집계)/실패/`s4_inventory` 미설정 3가지 경로 검증 |
