...
# ── 페이퍼 테스트 폭 넓히기 (M6) ─────────────────────────────────────
# 실거래(DRY_RUN=False)는 mode.max_positions(2) 그대로. 페이퍼에서만 확장.
# "2개 한세트로 10세트" = 20 포지션. 표본을 빨리 모으기 위함.
PAPER_POSITION_SETS    = 10     # 페이퍼 동시보유 = mode.max_positions × 이 값. 1이면 실거래와 동일
PAPER_ENTRIES_PER_TICK = 20     # 페이퍼: 한 틱에 신규 진입 최대 건수 (실거래는 항상 1)
