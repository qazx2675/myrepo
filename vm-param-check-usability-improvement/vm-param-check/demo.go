...
func runDemo(out, user string, onlyFail, noColor bool) {
	...
	shares := checker.SharesExpect{EV01: checker.RatioShares(demoExpectShares), Groups: map[string][]checker.SharesItem{"ev02": checker.RatioShares(demoExpectShares)}}
	...
	for _, vm := range vms {
		...
		findings = append(findings, checker.CheckHardware(vm, checker.CPUExpect{Base: demoExpectCPU}, checker.MemExpect{Base: demoExpectMemGB}, checker.DiskExpect{Base: []int{demoExpectDiskGB}}, shares, group, singleVMMode, false)...)
