...
func CheckTopology(vm model.VMInfo, cores CoresExpect, numa NumaExpect, group string, singleVMMode bool, isVcsim bool) []model.Finding {
	...
	if expectCores, ok := resolveGroupExpect(cores.Base, cores.Groups, group, singleVMMode); ok {
	...
	if expectNuma, ok := resolveGroupExpect(numa.Base, numa.Groups, group, singleVMMode); ok {
