...
func CheckHardware(vm model.VMInfo, cpu CPUExpect, mem MemExpect, disk DiskExpect, shares SharesExpect, group string, singleVMMode bool, isVcsim bool) []model.Finding {
	...
	// vCPU
	if expectCPU, ok := resolveGroupExpect(cpu.Base, cpu.Groups, group, singleVMMode); ok {
	...
	// 메모리 (GB)
	if expectMemGB, ok := resolveGroupExpect(mem.Base, mem.Groups, group, singleVMMode); ok {
	...
	// 디스크 (GB, 반올림)
	if expectDiskGB, ok := resolveGroupExpectList(disk.Base, disk.Groups, group, singleVMMode); ok {
...
func checkShares(vm model.VMInfo, shares SharesExpect, group string, singleVMMode bool) []model.Finding {
	items, ok := resolveGroupExpectShares(shares.EV01, shares.Groups, group, singleVMMode)
	if !ok {
		return nil
	}
