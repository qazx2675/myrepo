...
func main() {
	...
	affinityPaths := registerAffinityFlags()
	...
	if *specExport != "" {
		runSpecExport(*specRoot, *specExport)
		return
	}
	...
	buildExpect := func() expectSet {
		...
		e := expectSet{
			Cores:    checker.CoresExpect{Base: *cores, Groups: map[string]*int{}},
			Numa:     checker.NumaExpect{Base: *numa, Groups: map[string]*int{}},
			CPU:      checker.CPUExpect{Base: *cpu, Groups: map[string]*int{}},
			Mem:      checker.MemExpect{Base: *mem, Groups: map[string]*int{}},
			Disk:     checker.DiskExpect{Base: diskBase, Groups: map[string][]int{}},
			Shares:   checker.SharesExpect{EV01: sharesEV01, Groups: map[string][]checker.SharesItem{}},
			Affinity: map[string]map[string]string{},
			HTOn:     *ht == "on",
			PreferHT: *preferHT,
		}
		for _, g := range model.GroupNames()[1:] {
			for kind, dst := range map[string]map[string]*int{
				"cores": e.Cores.Groups, "numa": e.Numa.Groups, "cpu": e.CPU.Groups, "mem": e.Mem.Groups,
			} {
				v, err := parseOptionalIntFlag(kind+"-"+g, *groupFlags[kind][g])
				if err != nil {
					log.Fatal(err)
				}
				dst[g] = v
			}
			if e.Disk.Groups[g], err = parseIntListFlag("disk-"+g, *groupFlags["disk"][g]); err != nil {
				log.Fatal(err)
			}
			if e.Shares.Groups[g], err = parseSharesListFlag("shares-"+g, *groupFlags["shares"][g]); err != nil {
				log.Fatal(err)
			}
		}
		for _, g := range model.GroupNames() {
			...
			m, err := config.LoadAffinityFile(*affinityPaths[g])
			if err != nil {
				log.Fatalf("-affinity-%s 파일 읽기 실패: %v", g, err)
			}
			e.Affinity[g] = m
		}
		return e
...
func evaluateVM(vm model.VMInfo, e expectSet, singleVMMode bool) []model.Finding {
	...
	case "ev01":
		if a := e.Affinity["ev01"]; a != nil {
			f = append(f, checker.CheckAffinity(vm, a, "ev01")...)
		} else {
...
var specSettableFlags = func() map[string]bool {
	m := map[string]bool{
...
var groupFlagKinds = []struct{ kind, desc string }{
	...
	{"cpu", "vCPU 수"},
	{"mem", "메모리 GB"},
	{"disk", "디스크 총량 GB. -disk와 동일하게 쉼표로 여러 개 허용"},
...
func registerGroupFlags() map[string]map[string]*string {
	...
	for _, k := range groupFlagKinds {
		out[k.kind] = map[string]*string{}
		for _, g := range model.GroupNames()[1:] {
			out[k.kind][g] = flag.String(k.kind+"-"+g, "",
				fmt.Sprintf("기대값: %s 그룹 %s (옵션, 안 주면 %s %s 체크 스킵)", g, k.desc, g, k.kind))
		}
	}
	return out
}
...
func registerAffinityFlags() map[string]*string {
	...
	for _, g := range model.GroupNames() {
		usage := fmt.Sprintf("%s 그룹 기대 affinity 파일 (옵션, 안 주면 %s affinity 체크 스킵)", g, g)
		if g == "ev01" {
			usage = "ev01 그룹 기대 affinity 파일 (옵션. 안 주면 기존과 동일하게 -ht/-cores 기반 자동계산을 사용. 주면 파일값으로 대체)"
		}
		out[g] = flag.String("affinity-"+g, "", usage)
	}
	return out
}
...
// runSpecExport는 vCenter에 연결하지 않고 스펙을 VM 생성용 '이름=값' 줄로 출력한다.
// 표준출력은 vm_setup.sh가 그대로 파싱하므로 값 줄 외에는 아무것도 찍지 않는다(에러는 stderr + 종료코드 1).
func runSpecExport(specRoot, folderName string) {
	if specRoot == "" {
		log.Fatal("-specExport는 -specRoot와 함께 써야 합니다")
	}
	m, err := config.FindSpec(specRoot, folderName)
	if err != nil {
		log.Fatalf("스펙 조회 실패: %v", err)
	}
	if m == nil {
		log.Fatalf("폴더 %q에 해당하는 스펙 디렉터리를 %s 아래에서 찾지 못했습니다", folderName, specRoot)
	}
	lines, err := config.ExportSpec(m)
	if err != nil {
		log.Fatal(err)
	}
	for _, l := range lines {
