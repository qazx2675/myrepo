...
- **인터넷 불필요** — 의존성(`github.com/vmware/govmomi` 등)은 `V2/govendor/govmomi-0.39.0`에 있고 `setup.sh`가 `vendor`로 링크합니다. V2 폴더를 통째로 옮기면 폐쇄망에서도 바로 빌드됩니다. (폴더명 자동매칭 기능도 기존에 이미 vendor에 있던 `property` 패키지만 추가로 쓰므로, `vendor/`를 새로 채울 필요가 없습니다.)
