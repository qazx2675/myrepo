...
func main() {
	...
	for i := range flagsByGroup {
		s := fmt.Sprintf("ev%02d", i+1)
		flagsByGroup[i] = groupFlags{
			cores:   flag.Int(s+"Cores", 0, fmt.Sprintf("[%s] 코어 수 (미지정 시 %s 그룹은 처리하지 않음)", s, s)),
			sockets: flag.Int(s+"Sockets", 0, fmt.Sprintf("[%s] 소켓 수 (-%sCores 지정 시 필수)", s, s)),
			numa:    flag.Int(s+"Numa", 0, fmt.Sprintf("[%s] NUMA 노드 수 (0이면 토폴로지 NUMA 설정 생략)", s)),
		}
	}
	...
	rawGroups := make([]evGroup, maxGroup)
	for i, f := range flagsByGroup {
	...
	// 예전에는 finder.DefaultDatacenter()로 데이터센터 하나만 골라 VM을 찾았는데, 데이터센터가
	// 2개 이상이면 기본 데이터센터를 정하지 못해(에러도 무시돼서) VM 조회가 실패했다.
	// vCenter 최상위(RootFolder)부터 ContainerView(recursive)로 VM 이름을 1회 배치 조회하면
	// 데이터센터 수·그 아래 폴더 깊이와 무관하게 전부 찾는다(폴더를 한 단계씩 여는 finder보다 왕복도 적다).
	vmView, err := view.NewManager(client.Client).CreateContainerView(ctx, client.ServiceContent.RootFolder, []string{"VirtualMachine"}, true)
	if err != nil {
