...
func main() {
	...
	// 예전에는 DefaultDatacenter()로 하나만 골랐는데, 데이터센터가 2개 이상이면 기본값을 정하지
	// 못해(에러도 무시돼서) VM 조회가 실패했다. 데이터센터를 전부 돌면서 같은 방식으로 모은다
	// (데이터센터 1개면 예전과 같은 조회·같은 출력 순서).
	dcs, err := finder.DatacenterList(ctx, "*")
	if err != nil {
		log.Fatalf("데이터센터 목록 조회 실패: %v", err)
	}
	...
	var allVms []*object.VirtualMachine
	var lastErr error
	for _, dc := range dcs {
		finder.SetDatacenter(dc)
		vms, err := finder.VirtualMachineList(ctx, "*")
		if err != nil {
			lastErr = err // VM이 없는 데이터센터는 NotFound가 나므로 다른 데이터센터는 계속 본다
			continue
		}
		allVms = append(allVms, vms...)
	}
	if len(allVms) == 0 && lastErr != nil {
		log.Fatalf("VM 목록을 가져오는 중 오류 발생: %v", lastErr)
	}
